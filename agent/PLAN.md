...
- API key value is shown only once during creation.
...
- Reconnect uses exponential backoff with jitter and a maximum backoff of 60 seconds.
...
- If a subprocess needs a credential file, write it under runtime dir with `0600` permissions.
...
```bash
curl -fsSL https://<datamingle-host>/api/v1/agents/install.sh | sudo DATAMINGLE_URL="https://<datamingle-host>" DATAMINGLE_AGENT_API_KEY="<one-time-key>" bash
```
